...
class tinyimages {
	...
	function __construct() {
		...
		// Ajoutez ici la sйcuritй permettant de restreindre l'accиs а l'explorateur
	...
	private function leftPanel() {
		...
		// Trie les donnйes par volume dйcroissant, edition croissant
		// Ajoute $data en tant que dernier paramиtre, pour trier par la clй commune
	...
	private function mainField() {
		...
		if ($handle = opendir ( DIR.$this->folder )) {
			while (false !== ($file = readdir ( $handle ))) {
				if (is_file(DIR.$this->folder.'/'.$file)) {
					...
					if (is_file(DIR.$this->folder.'/'.$originales.'/'.$file)) {
						list($widthOrig, $heightOrig, $typeOrig, $attrOrig) = getimagesize(DIR.$this->folder.'/'.$originales.'/'.$file);
					}
					else {
						$widthOrig = 0;
					}
					$ret[] = array(
						'src'	=> $this->folder.'/'.$thumbs.'/'.$file,
						'orig'	=> $this->folder.'/'.$originales.'/'.$file,
						'attr'	=> $attr,
		...
		if(count($ret) > 0) {
			...
			foreach ($ret as $val) {
				...
				if ($val['widthOrig'] != 0)
					$addOrig = ' - <a href="#" title="Inserer l\'image originale" onclick="addImage(this,\''.$val['orig'].'\','.$val['widthOrig'].'); return false;" alt=" ">O</a>';
				else
					$addOrig = '';
				$return .= '<div class="item" ondblclick="addImage(this,\''.$val['path'].'\','.$width_for_insert.');" onclick="activateItem(this,\''.$val['path'].'\');"><img src="'.$val['src'].'" width="100" height="100" title="'.$val['name'].'"  alt=" " /><div class="labels"><a href="#" alt="" onclick="addImage(this,\''.$val['src'].'\',100); return false;" title="Inserer la miniature">M</a> - <a href="#"  alt=" " title="Inserer l\'image normale" onclick="addImage(this,\''.$val['path'].'\','.$width_for_insert.'); return false;">N</a>'.$addOrig.'</div><div class="labels">'.$val['name'].'</div><div class="labels">'.$val['width'].'x'.$val['height'].'</div><div class="labels">'.$val['size'].'</div></div>';
			}
		...
		if ($handle = opendir ( DIR.$input['dir'] )) {
			while (false !== ($file = readdir ( $handle ))) {
				if (is_file(DIR.$input['dir'].'/'.$file)) {
					...
					$thumb = $thumb_info['dirname'].'/'.$thumbs.'/'.$thumb_info['basename'];
					unlink($thumb);
		...
		if (isset($_FILES['photoupload'])) {
			...
			if ($error) {
				...
				if(!copy($_FILES['photoupload']['tmp_name'], $source)) {
					...
					$thumb = DIR.$this->folder.'/'.$thumbs.'/'.$name.'.'.$ext;
